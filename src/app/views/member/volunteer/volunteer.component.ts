...
export class VolunteerComponent implements OnInit {
  ...
  GetOrganizationData() {
    this.volunteerService.GetAllVolunteerOrganization().subscribe((data) => {
      if (data.length > 0) {
  ...
  showList() {
    this.volunteerService.GetVolunteerOpportunityByCause(this.selectedCauseID, this.lastVisible).subscribe((data) => {
      if (data.length > 0) {
