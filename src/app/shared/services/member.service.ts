...
export class MemberService {
  ...
  getAllExpiredDeletionList() {
    return this.firestore
      .collection("deletion", (ref) => ref.where("mdate", "<" , new Date() ))
      .snapshotChanges();
  }

  getDeletionList() {
    return this.firestore
      .collection("deletion", (ref) => ref.orderBy("mdate", "asc").limit(31))
      .snapshotChanges();
  }
  ...
  GetData(doc: any) {
    if (doc != null) {
      return this.firestore
        .collection("users", (ref) => ref.startAfter(doc).limit(31))
        .snapshotChanges();
    } else {
      return this.firestore
        .collection("users", (ref) => ref.limit(31))
        .snapshotChanges();
    }
  }
}
